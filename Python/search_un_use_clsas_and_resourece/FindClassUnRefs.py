...
RESULT_FILE_PATH = sys.path[0].strip() + '/' + 'find_class_unRefs.txt'
...
def pointers_from_binary(line, binary_file_arch):
    ...
    if binary_file_arch.startswith('arm'):
        ...
        return pointers  # [0000000103138580, 0000000103138878]
    return None
...
def filter_super_class(path, unref_symbols):
    re_subclass_name = re.compile("\w{16} 0x\w{6,9} _OBJC_CLASS_\$_(.+)")
...
def find_unref_symbols(paths):
    ...
    for unref_pointer in unref_pointers:
        if unref_pointer in all_symbols:
            unref_symbol = all_symbols[unref_pointer]
...
# 自己作为自己的属性不算
def find_ivars_is_unuse_class(path, unref_symbols):
    # {'MyTableViewCell':
...
def write_to_file(unref_symbols, find_ivars_class_list, moudle):
    if len(unref_symbols) == 0:
        return
...
def new_logic(paths):
    ...
    for path in paths:
        ...
        # 查找当前未被引用的子类
        unref_symbols = filter_super_class(path, unref_symbols)
