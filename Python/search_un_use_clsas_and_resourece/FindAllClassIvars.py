...
def get_all_class_ivars(path):
    ...
    re_ivars_start = re.compile('\sivars\s*0x\w{6,9}\s*__OBJC_\$_INSTANCE_VARIABLES_(.+)')
    #  weakIvarLayout 0x0
    re_ivars_end = re.compile('\sweakIvarLayout\s0x0')
    re_ivar_name = re.compile('\s*name\s*0x\w{6,9}\s*_(.+)')
    re_ivar_type = re.compile('\s*type\s*0x\w{6,9}\s*@\"(.+)\"')
    imp_ivars_info = {}
    is_ivars_area = False
